...
-- Database: sap_integration_db

CREATE EXTENSION IF NOT EXISTS pgcrypto;

-- Inbox pattern: guarantees idempotent consumption of SQS messages.
...
-- Persistent state of the integration with SAP.
CREATE TABLE IF NOT EXISTS sap_sync_operations (
    ...
    CONSTRAINT ck_sap_sync_attempts_non_negative
        CHECK (attempts >= 0)
);
...
-- Outbox for supplier.sap-sync.completed.v1 / failed events.
